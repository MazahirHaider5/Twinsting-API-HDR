...
export const becomeArtist = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const user = await User.findByIdAndUpdate(userId, { role: "artist" }, { new: true });
    if (!user) {
      res.status(404).json({
        success: false,
...
export const artistSkills = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const artist = await Artist.findOneAndUpdate({ user: userId }, { $set: { skills } }, { new: true });
    if (!artist) {
      res.status(404).json({
        success: false,
...
export const getAllArtists = async (req: Request, res: Response): Promise<void> => {
  try {
    const artists = await Artist.find().populate("user", "profilePicture"); // Use 'profilePicture' as per User model
    res.status(200).json({
      success: true,
...
export const getArtistById = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const artist = await Artist.findById(artistId).populate("user", "profilePicture");
